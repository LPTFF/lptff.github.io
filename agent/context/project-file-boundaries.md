...
这里放产品定位、PRD、业务规划、研究、长期记录、代码组织说明、验收方法和其他只供维护者阅读的材料。
...
禁止 `src/`、Vite 配置、npm scripts、扩展打包器或 CI import、执行或发布 `agent/` 内容。不得在 `agent/` 复制一份可执行脚本或运行时代码，以免产生漂移。
...
- `project-support/public/**`
- `src/data/**`、`src/assets/**`
- `src/content/investment/investment-assistant.md`
