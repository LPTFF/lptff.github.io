...
- 技术栈：Vue 3、Vue Router、Vite、JavaScript、TypeScript；npm 管理依赖；具体版本以 `package.json` 与锁文件为准。
...
- 2026-09-10 用户确认现有功能基本满足需要，当前以维护为主；方向和取舍统一见[产品工作台](../product/README.md)。不从“资源储备”概念自动新增台账、任务中心或结果记录系统，不设投资优先的全站建设门槛。
- Contract Review 继续保留为冻结的验证原型：不提供默认交易参数、安全或执行许可结论；重新评估依据该领域实际需求与证据，而非其他领域的复盘次数。
- 当前 `/career` 为独立求职机会发现页面，`/boss-zhipin` 重定向到它；页面提供简历画像、搜索方向与公开招聘快照，浏览器插件承担现场能力。旧 LeetCode、技术论坛、GitHub Trending 和独立面试页面重定向到博客；不能把旧页面归档误写成当前求职能力不存在。
- 高级搜索与资讯文章目前为归档内容，历史页面重定向到博客；不在维护任务中自动恢复旧页面。与“薅羊毛”现有消费者直接相关的公开关键词检索在后台采集和发布，其余归档采集器只在明确任务中手动按需运行。
- 旧加密货币策略展示页已退出一级产品，`/cryptocurrency` 重定向到产品复盘文章；剩余旧基金持仓页 `/fundHoldInfoMsg` 已直接删除，不并入 Investment Review。
- 福利使用固定公开页面/API 采集与 Google `site:` 定向发现（GitHub、Telegram、Bilibili），页面合并去重并关联 Gemini 生态标注，按类别和来源筛选。条目数表示收录资讯，不代表活动资格或有效性已核实；精确采集范围和处理规则以当前配置、处理器及页面消费者为准。
- 复盘领域可采用“记录 → 分析 → 改进 → 验证”，不要求导航、资讯、工具和休闲功能实现同一工作流；不把产品判断问题转换为用户必填表单。
- 通用聊天壳、任务中心、资源台账和复杂 Agent 系统没有当前建设需求；不因概念完整性自动推进。
...
- `project-support/extension/lptff-investment-assistant/` 是项目功能；`project-support/scripts/` 是构建与内容同步链路。公开站点采集与分类属于 qinglongBackup/src/site_crawlers；本站只导入 python-crawl 数据。
...
- 来源完整性须依据实际页集合、总数与跨页重复校验；旧快照或历史通过记录不能证明当前采集完整。
- 本文不维护最新验收状态；结论必须绑定本次代码、环境与场景，具体证据范围见[真实环境验收原则](../standards/trusted-verification.md)。
