...
- 职业方向以真实招聘市场为输入、浏览器插件为现场入口、AI 为信息和知识加工层；LeetCode、技术论坛、GitHub Trending、静态 BOSS 列表和独立面试页面已退出一级产品。高级搜索与资讯文章也已归档：检索、筛选和总结改为 AI 按需完成，历史页面重定向到博客，相关采集器只允许手动按需运行。
- 产品闭环：记录 → 分析 → 改进 → 验证。
