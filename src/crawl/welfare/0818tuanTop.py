...
import base64
# 发起 get 请求
url = 'http://www.0818tuan.com/list-2-0.html'
headers = {
...
try:
    ...
    if response.status_code == 200:
        ...
        box_div = soup.find('div', class_='list-group')
        ...
        if box_div:
            for article in box_div.find_all('a',class_="list-group-item"):
                if article.find('span', class_='badge badge-success red'):
                    ...
                    # 获取当前日期
                    today = datetime.now(beijing_tz).strftime('%Y').strip()+'-'
        ...
        # 保存为JSON文件
        with open('./src/public/data/welfare/0818tuanTop.json', 'w', encoding='utf-8') as file:
            file.write(json_data)
            print('0818tuanTop分析数据导出成功')
    else:
        print('0818tuanTop请求失败001')
except requests.exceptions.RequestException as e:
    print('0818tuanTop请求异常002:', e)
except Exception as e:
    print('0818tuanTop发生异常003:', e)
