  ...
  <div class="fund-import">
    <el-card shadow="never" class="plugin-card">
      ...
      <div class="plugin-intro">
        先在天天基金正常登录，再点击浏览器插件的“同步最近投资变化”。v2 插件会把已采集的事实暂存到本地，回到 Investment OS 点击“读取插件数据”写入 IndexedDB；旧版 <code>fund-data.json</code> 仍作为备份、调试和迁移入口。
      ...
      <ol class="plugin-steps">
        ...
        <li>插件会并行加载少量基金详情，并行查询当前/历史交易范围；每个交易页面内部仍按顺序加载实际分页，未加载的历史不会标记为完整。</li>
        <li>任务完成后 v2 数据留在插件本地 staging；回到新版 Investment OS 点击“读取插件数据”完成本地 Ledger 同步，同时仍会下载 <code>fund-data.json</code> 作为备份。</li>
        <li>需要查看旧版复盘时，再将 <code>fund-data.json</code> 拖入下方导入区。</li>
      </ol>
    </el-card>
...
async function onChange(file: UploadFile) {
  ...
  try {
    ...
    if (r.ok && r.data) {
      ...
      setTimeout(() => router.push("/investment/legacy"), 600);
    } else if (r.data) {
...
async function useExample() {
  try {
    ...
    if (r.data) {
      ...
      setTimeout(() => router.push("/investment/legacy"), 500);
    }
  } catch (e) {
