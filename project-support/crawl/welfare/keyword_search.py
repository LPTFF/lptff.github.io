...
def build_search_url(anchors: list[str], benefits: list[str], window_days: int) -> str:
    ...
    query = f"({anchor_query}) ({benefit_query}) when:{window_days}d"
    ...
    *,
    ...
    for item in root.findall("./channel/item")[:MAX_RESULTS_PER_QUERY]:
        ...
        try:
            ...
            continue
        if published_at < cutoff or urlparse(link).hostname != "news.google.com":
            continue
...
def fetch_candidates(config: dict[str, object]) -> list[Candidate]:
    ...
    for query in config["queries"]:
        ...
        url = build_search_url(anchors, benefits, window_days)
