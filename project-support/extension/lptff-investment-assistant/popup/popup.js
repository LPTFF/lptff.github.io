...
function branchProgress(branch) {
  ...
  return `${state}${count ? ` ${count}` : ""}${orderSources}${deduplicated}${duration}`;
}
...
const OBSERVATION_PLATFORMS = [
  ...
  },
  {
    ...
    loginHint: "登录快手 → 打开目标主页或推荐页 → 开始观察 → 滚动几屏（触发视频列表与分页游标，作者信息通常内嵌在响应里）→ 等倒计时结束",
  },
  {
    id: "douyin",
    name: "抖音",
    group: "entertainment",
    intro: "采集抖音推荐流/主页响应，生成视频（文案/封面/播放地址/统计/时长）、作者和分页上下文正式来源包。目标与快手一致：离线清单高效消费。",
    loginHint: "登录抖音 → 打开推荐页或目标主页 → 开始观察 → 滚动几屏（触发视频列表与分页游标，作者信息通常内嵌在响应里）→ 等倒计时结束",
  },
];
...
function buildObservationCard(platform) {
  ...
  title.textContent = `${platform.name} · 观察采集`;
  ...
  summary.textContent = "观察与导出设置";
  const durationLabel = document.createElement("label");
  durationLabel.textContent = platform.id === "binance" ? "自动采集时长（固定 30 秒） " : "观察时长(秒, 30-600) ";
  ...
  sourceBackup.textContent = "下载正式来源包";
  ...
  sourceDesensitize.textContent = "下载脱敏正式来源包";
  ...
  discard.textContent = "丢弃当前报告";
  const hint = document.createElement("p");
  hint.className = "action-hint";
  hint.textContent = `登录调试建议：${platform.loginHint}。正式来源包只含产品白名单实体；观察报告用于核对端点契约。「完整本地备份」含业务标识，仅限本地。`;
  advanced.append(summary, durationLabel, saveConfig, sourceBackup, sourceDesensitize, backup, desensitize, discard, hint);
  card.append(title, intro, actions, status, advanced);
  document.querySelector(`#obs-group-${platform.group}`).append(card);
  return { card, status, start, stop, sourceBackup, sourceDesensitize, backup, desensitize, discard, duration, saveConfig };
}
...
function updateObservationCard(platformId, progress) {
  ...
  if (progress.running) {
    ...
    const suffix = progress.stage === "observing" ? `剩余 ${Math.ceil(remaining / 1000)} 秒` : "";
    const stageLabel = platformId === "binance" && progress.stage === "observing"
      ? "后台自动采集中，无需操作页面"
    ...
    ui.stop.disabled = platformId === "binance" || progress.stage !== "observing";
  ...
  if (progress.stage === "error") {
    ...
    setObservationStatus(platformId, `观察失败：${warning || "未知错误"}`, "error");
  }
  refreshObservationStatus();
}
  ...
  try {
    ...
    for (const [platformId, state] of Object.entries(platforms)) {
      ...
      if (state.pending) {
        ...
        setObservationStatus(platformId, `采集报告已就绪：REST 端点 ${summary?.restEndpointCount || 0} 个 · WS 流 ${summary?.wsStreamCount || 0} 个 · 覆盖 ${observed}/${total} 类${warnings}。${sourceHint}${coreSummary}${coreMissingHint}可下载正式来源包、观察报告或丢弃。`, warnings || missingCore.length || !entitySummary ? "" : "success");
        continue;
      }
...
async function startObservation(platformId) {
  ...
  if (!ui) return;
  ui.start.disabled = true;
  setObservationStatus(platformId, "正在启动观察…");
  try {
    ...
    if (!response?.ok) throw new Error(response?.error || "观察启动失败");
    updateObservationCard(platformId, { running: true, stage: "observing", remainingMs: response.durationMs });
    refreshObservationStatus();
  } catch (error) {
    ui.start.disabled = false;
    setObservationStatus(platformId, error instanceof Error ? error.message : "观察启动失败", "error");
  }
}
...
async function stopObservation(platformId) {
  ...
  if (!ui) return;
  ui.stop.disabled = true;
  setObservationStatus(platformId, "正在读取观察数据…");
  try {
...
for (const [platformId, ui] of observationCards) {
  ...
  ui.sourceBackup.addEventListener("click", () => exportObservationData(platformId, "EXPORT_OBSERVATION_SOURCE_BACKUP", "正式来源包已生成。"));
  ui.sourceDesensitize.addEventListener("click", () => exportObservationData(platformId, "EXPORT_OBSERVATION_SOURCE_DESENSITIZED", "脱敏正式来源包已生成。"));
  ui.discard.addEventListener("click", async () => {
    try {
      ...
      setObservationStatus(platformId, "观察报告已丢弃，可重新观察。");
    } catch (error) {
  ...
  ui.saveConfig.addEventListener("click", async () => {
    try {
      ...
      setObservationStatus(platformId, "观察设置已保存（对所有观察平台生效），下次观察生效。", "success");
    } catch (error) {
