...
- 本地验证报告按“当前 + 归档”管理：`docs/verification-reports/current/` 只保存仍与当前代码相关的中文结论报告，关联原始证据位于 `current/assets/`；已结束批次移至 `archive/YYYY-MM-DD/`。`latest.md` 自动汇总当前结论、证据规模和归档规模，不以文件名字母顺序作为最新状态。
