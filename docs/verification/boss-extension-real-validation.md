...
目标：把 `extension/` 的每项 UI 承诺转成可观察结果，在 Windows Chrome 的真实 BOSS 页面中完成“复现 → 修复 → 重载 → 重测”，直到本次变更涉及的场景收敛。
...
- **本地开发直接加载源码目录**：日常本地调试使用 Chrome“加载已解压的扩展程序”或对已加载目录点“重新加载”，目标目录是仓库中的 `extension/`。ZIP、发包和线上发布只在用户明确要求发布，或真实消费者必须依赖构建产物时执行；不能把发包当成本地调试前置条件。
...
2. 核对扩展名称、版本、ID和加载目录，目录必须是当前工作区的 `extension/`。
