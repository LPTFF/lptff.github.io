...
- **Changed files**：
  - `extension/manifest.json`：扩展版本升级到 `3.17.15`。
  - `extension/content/boss-autopilot.js`：
...
- **Changed files**：
  - `extension/manifest.json`：扩展版本升级到 `3.17.13`。
  ...
  - `extension/dist-extension/lptff-investment-assistant.zip`：重新打包生成（`730474` 字节）。
...
- **Changed files**：
  ...
  - `extension/manifest.json`：更新 `content/web-bridge.js` 匹配范围，覆盖 `http://localhost/*` 与 `https://lptff.github.io/*`。
  - `extension/content/web-bridge.js`：新增 `LPTFF_CAREER_*` 消息监听与安全中继。
  - `extension/background.js`：新增求职助手消息分发、Gemini 简历提取（提示注入防御与原文出处抽取）、市场方向匹配（程序化硬校验佐证岗位与原文片段）、权威本地存储与受控同步至 BOSS 沟通助手画像。
  - `.local/generated/crawl/zhipin.py`：补充 `capturedAt`、`pageUpdatedAt`、`jobPostTime`、`sourceStatus` 等元数据。
  - `.local/generated/crawl/run_collectors.py`：将 `zhipin` 纳入每日全量 CI 采集（`group="full"`，`optional=True`）。
...
- **构建与测试验证**：
  ...
  - `node scripts/extension/build-zip.js`：**PASS**（打包生成 `dist-extension/lptff-investment-assistant.zip`）。
  - `python -B .local/generated/crawl/zhipin.py`：**PASS**（成功采集 25 城公开页样本并生成 `src/data/zhipin.json`）。
...
- **验证执行环境与定位排查**：
  ...
  - **扩展程序**：ID `ajmbdhnpebogbcphaaapjjgocccifgni`（LPTFF Investment Assistant 3.17.12，加载自 `extension`）。
...
- **真实环境核验（非独立测试环境）**：
  ...
  - 扩展 ID：`ajmbdhnpebogbcphaaapjjgocccifgni`（LPTFF Investment Assistant 3.17.12，未打包源码加载自 `extension`）。
...
- Changed files：`extension/boss.js`（按需连接改造、未登录受控降级、统计除零安全兜底）、`extension/manifest.json`（v3.17.12）。
...
- Changed files：`extension/boss.js`（按需连接生命周期、10s 超时、失败恢复、未登录防守与统计除零兜底）、`extension/manifest.json`（版本 3.17.11）、`extension/content/boss-devtools-shield.js`、`rules/boss-devtools.json`。
- Impacted behaviors：
...
- Changed files：`manifest.json` 升为 3.17.11，新增 `content/boss-devtools-shield.js`、`rules/boss-devtools.json`（均位于扩展源码目录）；优化 `extension/boss.js` 以及上游源码 `docs/references/boss-helper-upstream/src/components/Tabs/Statistics.vue` 与 `src/entrypoints/boss/chat/index.ts`、`src/entrypoints/boss/index.ts`；同步根 AGENTS、验收原则、本手册状态和 BOSS 调试文档。
- Impacted behaviors：
...
- `.local/generated/crawl/welfare/keyword_search.py`：新增 Google 新闻关键词 RSS 采集、时效/去重、Gemini 结构化分类、ID 完整性校验和本地权益信号护栏。
- `.local/generated/crawl/welfare/keyword_search_config.json`、`src/data/welfare/keyword-search.json`：新增可审查的四组采集词和 AI 准入快照。
- `src/views/home/welfare/index.vue`：消费关键词采集结果，显示真实来源并纳入现有薅羊毛排序。
- `.local/generated/crawl/run_collectors.py`、`.github/workflows/ci.yml`：注册采集器并加入每小时福利采集。
...
- `docs/references/boss-helper-upstream/src/App.vue`：工作台更名为“自动投递”，移除上游 AI/Chat 入口，只保留统计、筛选、配置和日志。
...
- 运行时脱敏数据从 `project-support/fixtures/` 移至 `data/snapshots/`，明确其为产品数据而非测试替身。
