...
`docs/` 用来降低理解、判断和验证成本，不是 Agent 的控制面。执行原则以根目录 [AGENTS.md](../AGENTS.md) 为准；这里只提供项目事实和任务入口。
...
- **比较有依据**：案例保留最小输入、来自业务规则或已核验结果的预期、检查方式；评测逻辑与样本就近放在对应代码或 `scripts/`，不放在 `docs/`。比较模型时固定案例和条件，记录实际模型、代码或提示版本；波动结果按需复测，未知费用不估作事实。高级模型可分析失败，不能独自生成答案又作为唯一裁判。
