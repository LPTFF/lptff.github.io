...
## 2026-09-08 — BOSS 扩展未登录异常消除、统计 NaN 修复与真实日常 Chrome 验收闭环

- 触发：用户反馈扩展在浏览职位页时控制台与扩展错误页抛出 `Uncaught (in promise) Error: 消息发送ws: 获取 wt 失败: 当前登录状态已失效`（`boss.js:1040`），且自动投递工作台的过滤比例、重复比例、活跃比例出现 `NaN %`。
- 根因：
  1. 聊天 WebSocket 连接原设计在组件挂载阶段自启动，未登录时获取 `wt` 凭据失败抛出 Promise 拒绝且缺乏未捕获防守，被 Chrome 扩展记录至错误详情页。
  2. 统计卡片计算 `(count / total * 100)` 未做分母除以零保护，初始态岗位总数为 0 时产生 `NaN %`。
- 方案：
  1. 聊天 WebSocket 改为按需惰性触发，浏览职位阶段绝不发起握手；未登录或 token 失效时受控退出并提示，杜绝 Promise 泄漏。
  2. 统计指标计算增加除零防守，兜底显示 `0 %`。
- 环境与交付原则：
  1. 确立以用户日常使用、已有扩展和真实登录态的 Chrome（Default Profile）为唯一准绳；此前独立沙箱测试（如 `chrome-normal-profile`）明确标注为“独立环境测试”，严禁伪装。
  2. 严禁自写测试脚本（如 `test-*.js`、`.ps1` 辅助脚本等）作为项目产物提交，只认真实环境执行的脱敏事实与截图证据。
- 验证结果：在日常 Chrome 中清空历史堆栈并重新加载扩展后，刷新真实 BOSS 职位页，工作台挂载正常，统计卡片显示正常比例，错误页严格维持 0 报错；零外发、零通知。

## 2026-08-07 — Agent 资料转为人的工作台

- 触发：原有 `docs/` 内容逐渐变成强制阅读顺序、固定生命周期、机器检查和 Agent 管理模板，让人为了工具维护格式。
- 决定：删除外部 Agent 协作模板、治理脚本、验证报告索引和恢复记录；保留项目事实、产品判断、研究资料和简短验收建议。
- 原则：人定义问题和取舍，Agent 只加速搜索、比较、起草和可逆验证；所有流程都是可选建议，不能成为人的工作门槛。
- 结果：`docs/README.md`、`standards/`、`context/project-context.md` 和 `verification/playbook.md` 已改成人可直接使用的资料；业务代码和现有扩展改动未纳入本轮。
...
- 决定：Agent A 默认拥有纵向任务，从需求、设计、实现、迁移和 UI 到真实环境排错、构建诊断、修复与非私人验证，常态承担 80% 以上工作；Agent B 不是通用 reviewer，只在 A 已完成可完成部分，而关键主张仍只能由人授权的私人数据或真实登录环境证明时，做一次窄范围、只读、脱敏验证。
- 人的边界：维护者定义结果和规则，批准 B 的目标、环境、数据、动作与停止条件，并裁决价值冲突和剩余风险。授权被拒绝或证据无法安全取得时保持 `BLOCKED/UNKNOWN`，不得让 A 猜测或让 B 扩权。
- 资源原则：“80% 以上”不建立工时配额；B 是否启用取决于权限隔离造成的真实证据缺口，而不是代码复杂度、高风险标签或希望再找一个 Agent 重看。B 返回失败后，由 A 依据已观察的来源事实修复，必要时再经人授权在同一目标环境定点复验。
