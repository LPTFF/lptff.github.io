...
- 页面位于 `src/views/`，页面内容与唯一消费者放在一起：博客文章、职业方向和面试归档位于 `src/views/Blog/articles/`，投资协议位于 `src/views/investment/`；应用数据位于 `src/data/`，应用资源位于 `src/assets/`；扩展、脚本、公开资源和构建配置分别位于根目录同名目录。
...
- 维护资料与运行时文件按实际消费者隔离：`docs/` 只供维护者阅读，禁止 `src/`、Vite、npm scripts 或 CI 依赖它；边界与不可移动清单见 `docs/context/project-file-boundaries.md`。
- `extension/` 是项目功能；`scripts/` 是构建与内容同步链路。公开站点采集与分类属于 qinglongBackup/src/site_crawlers；本站只导入 python-crawl 数据。
...
- 仓库中的 `extension/` 是扩展唯一维护源。真实诊断曾发现 Chrome 加载的是仓库外的解压副本；重载前必须在扩展管理页核对实际加载目录，并以文件清单或哈希确认它与仓库源码一致。
