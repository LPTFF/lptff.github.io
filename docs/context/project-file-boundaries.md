...
会被 Vite、npm、CI、扩展发布或数据链路消费的支持代码和资源，按用途直接使用根目录同名入口：
...
这里放产品定位、PRD、业务规划、研究、长期记录、代码组织说明、验收方法和其他只供维护者阅读的材料。`docs/references/` 可以保存固定版本的外部参考源码用于差异分析，但不能成为产品构建或运行依赖。
...
没有运行时消费者的维护文档可以放在 `docs/docs/`。根 README 和文档相对链接必须同步更新。
