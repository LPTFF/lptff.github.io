...
已验证：全部脚本 `node --check` 通过；manifest JSON 校验通过（v3.2.0，5 组 content_scripts）；`node scripts/extension/build-zip.js` 成功生成 `dist-extension/lptff-investment-assistant.zip`（46703 bytes）。
...
前提：Chrome 已加载新 zip（chrome://extensions → 加载已解压的扩展程序指向 `extension/`，或直接加载 zip）。
