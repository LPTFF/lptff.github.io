...
`extension/` 是项目功能；`scripts/extension/build-zip.js` 是本地下载和 Release 打包链路；二者不能搬入 `docs/`。`src/views/investment/investment-assistant.md` 虽是 Markdown，但被 `FundImport.vue` 以 `?raw` 编译进用户可见页面，必须留在运行时可读路径。
