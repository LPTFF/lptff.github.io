...
以下是项目功能或交付链路，不能移动到 `docs/`：

- `src/`、`public/`、`.local/generated/crawl/`、`extension/`
- `scripts/extension/build-zip.js`
- `scripts/sync-findJob-summary.js`、`scripts/copy-404.js`
- `scripts/collectors/**`
...
- `npm run collect:rss:site`：生成被 Git 忽略的 `public/data/recommendArticleData.json`。
- `node scripts/extension/build-zip.js`：生成被忽略的 `dist-extension/lptff-investment-assistant.zip`。
